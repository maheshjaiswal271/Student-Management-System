...
def DisplayForm():
    ...
    root.title("Student Management System (BY Mahesh)")
    global tree
    global SEARCH
    ...
    #Heading
    lbl_text = Label(TopViewForm, text="Student Management System", font=('Arial Bold', 25), width=700,bg="red",fg="white")
...
def SearchRecord():
    ...
    if SEARCH.get() != "":
        tree.delete(*tree.get_children())
        cursor=conn.execute("SELECT STU_NAME,STU_CONTACT,STU_EMAIL,STU_ROLLNO,STU_BRANCH FROM STU1 WHERE STU_NAME LIKE ?", ('%' + str(SEARCH.get()) + '%',))
